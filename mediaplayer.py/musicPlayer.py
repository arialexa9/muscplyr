...
def increase_volume():
    try:
        ...
        if current_volume >=1:
            ...
            return
        current_volume = current_volume - float(0.2)
        current_volume + round(current_volume,1)
        mixer.music.set_volume(current_volume)
        volume_label.config(fg="green",text="Volume : " +str(current_volume))
    except Exception as e:
...
Label(master, text="Please select a music track you like to play",font=("Caliibri", 12),fg="blue").grid(sticky="N",row=0,padx=1)
Label(master, text="Volume",font=("Caliibri", 12),fg="red").grid(sticky="S",row=4)
...
Button(master, text="Pause",font=("Calibri",12)).grid(row=3,sticky="E")
Button(master, text="Resume",font=("Calibri",12)).grid(row=3,sticky="W")
